...
* **Declarative Querying (SQL / DML):** Formulating multi-table joins (inner, outer, cross), correlated nested subqueries, aggregations with `GROUP BY` / `HAVING`, and window functions.
* **Schema Definition & Integrity (DDL):** Writing schema migration scripts, enforcing referential integrity constraints, table constraints, and check conditions.
* **Procedural Automation & Triggers:** Developing procedural business rules using stored procedures, user-defined functions, and event-driven triggers in PL/SQL / procedural SQL.
* **Performance & Physical Storage:** Indexing strategies (B-Trees, Hash indexes), query execution plan evaluation, and transaction management concepts (ACID properties, concurrency control).
...
* **Modeling & Design:** ER Diagrams, Relational Schema Mappings
* **Database Tools:** Oracle SQL Developer / Oracle Apex
